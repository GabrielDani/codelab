...
if (
  ...
  throw new Error(
    "CLOUDFLARE_ACCOUNT_ID, CLOUDFLARE_ACCESS_ID, CLOUDFLARE_ACCESS_KEY, CLOUDFLARE_R2_BUCKET_NAME e CLOUDFLARE_FILE_BASE_PATH devem ser definidos"
  );
}
...
export const uploadFile = async ({ file, path }: UploadFileParams) => {
  ...
  if (size > fileMaxSize) {
    throw new Error("O arquivo deve ser menor que 5MB");
  }
