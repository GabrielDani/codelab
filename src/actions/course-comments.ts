...
export const createLessonComment = async ({
  ...
  if (!userHasCourse)
    throw new Error("Você precisa comprar o curso para comentar");
      ...
      await prisma.notification.create({
        data: {
          ...
          content: `${formatName(
            user.firstName,
            user.lastName
          )} respondeu ao seu comentário: "${content}" no curso ${
            course.title
          }`,
          link: `/courses/${courseSlug}/${lesson.moduleId}/lesson/${lesson.id}`,
        },
      });
    }
  }
