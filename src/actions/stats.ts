...
export const getNewUsersStats = async (): Promise<StatsChartData[]> => {
  ...
  return lastSevenDays.map((date) => ({
    date,
    count: usersCount.get(date.toISOString().split("T")[0]) || 0,
  }));
};

export const getNewPurchasedCoursesStats = async (): Promise<
  StatsChartData[]
> => {
  const sevenDaysAgo = new Date();
  sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
  ...
  return lastSevenDays.map((date) => ({
    date,
    count: purchasesCount.get(date.toISOString().split("T")[0]) || 0,
  }));
};
