...
export const CommentItem = ({
  ...
  const { mutate: handleDeleteComment, isPending: isDeleting } = useMutation({
    ...
    onError: () => toast.error("Erro ao deletar comentário"),
  });
  ...
  return (
    <div
      className={cn(
        "flex flex-col gap-3 text-sm p-4 rounded-lg bg-card",
        className
      )}
    >
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <Avatar src={comment.user.imageUrl} fallback={authorName} />
      ...
      {isReplying && (
        <CommentInput
          ...
          className="bg-muted p-4 rounded-lg flex flex-col sm:flex-row"
        />
      )}
    </div>
  );
